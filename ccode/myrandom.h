...
int32_t generate_normalized_fp32() {
    //生成高质量随机种子，并且通过static来保证函数只初始化一次，这个是真随机数，其从操作系统/硬件中获取随机数
    ...
    //&float_bits：获取uint32_t数据的地址；(int32_t *)：将uint32_t *的指针类型改为int32_t *的指针。最后一个*会读取该地址中的值
...
int16_t generate_normalized_fp16() {
    ...
    std::uniform_int_distribution<uint16_t> exp_dist(0, 31); 
...
int16_t generate_normalized_bf16() {
    ...
    // 规格化数的指数范围：1-254（避免0和255）
    std::uniform_int_distribution<uint16_t> sign_dist(0, 1);
    std::uniform_int_distribution<uint16_t> exp_dist(0, 255); 
...
// 生成随机规格化_scale ue4m3（8位）
int8_t generate_normalized_ue4m3() {
    static std::random_device rd;
    static std::mt19937 gen(rd());

    std::uniform_int_distribution<uint16_t> bit7(0, 126); // e4m3只有低7位全1是NAN

    uint16_t ue4m3_bits = bit7(gen);
    return *(int8_t*)&ue4m3_bits;
}
...
// 生成随机规格化__MXFP6 e3m2（6位）
int8_t generate_normalized_e3m2() {
    static std::random_device rd;
    static std::mt19937 gen(rd());

    std::uniform_int_distribution<uint16_t> sign_dist(0, 1);
    std::uniform_int_distribution<uint16_t> exp_dist(0, 7);
    std::uniform_int_distribution<uint16_t> mantissa_dist(0, 3);

    uint16_t sign = sign_dist(gen);
    uint16_t exp = exp_dist(gen);
    uint16_t mantissa = mantissa_dist(gen);
    uint16_t e3m2_bits = (sign << 5) | (exp << 2) | mantissa;
    return *(int8_t*)&e3m2_bits;
}

// 生成随机规格化__MXFP6 e2m3（6位）
int8_t generate_normalized_e2m3() {
    static std::random_device rd;
    static std::mt19937 gen(rd());

    std::uniform_int_distribution<uint16_t> sign_dist(0, 1);
    std::uniform_int_distribution<uint16_t> exp_dist(0, 3);
    std::uniform_int_distribution<uint16_t> mantissa_dist(0, 7);

    uint16_t sign = sign_dist(gen);
    uint16_t exp = exp_dist(gen);
    uint16_t mantissa = mantissa_dist(gen);
    uint16_t e2m3_bits = (sign << 5) | (exp << 3) | mantissa;
    return *(int8_t*)&e2m3_bits;
}


int8_t generate_normalized_ue8m0() {
    static std::random_device rd;
    static std::mt19937 gen(rd());

    std::uniform_int_distribution<uint16_t> bit8(0, 255);//8b'1111_1111为NAN
    return bit8(gen);
}
...
//生成单个e2m1，但是其存储在一个uint8中，且放置在fp8的中心
int8_t generate_normalized_e2m1() {
    static std::random_device rd;
    static std::mt19937 gen(rd());

    std::uniform_int_distribution<uint16_t> bit4(0, 15);

    uint16_t bit4_random = bit4(gen);
    uint16_t e2m1_bits = bit4_random << 2;

    return *(uint8_t*)&e2m1_bits;
}
...
int gen_exception_fp6 (int8_t* a, int8_t* b, int32_t* c, int size, int is_rand){
    return 0;
};